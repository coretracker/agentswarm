...
const task = (ownerUserId: string, repoId = "repo-1"): Task => ({ ownerUserId, repoId } as Task);

test("task access keeps owners and admins, and admits only repository-authorized teammates", () => {
  assert.equal(canUserAccessTask(user(), task("user-1", "repo-2")), true);
  assert.equal(canUserAccessTask(user(), task("user-2")), true);
  assert.equal(canUserAccessTask(user(), task("user-2", "repo-2")), false);
  assert.equal(canUserAccessTask(user(), task("user-3")), false);
  assert.equal(canUserAccessTask(user({ teamMemberUserIds: [] }), task("user-2")), false);
...
test("task listing combines own tasks with repository-authorized teammate tasks", async () => {
  const tasks = [
    ...
    { ...task("user-2"), id: "team", pinned: false, createdAt: "2026-01-02T00:00:00.000Z" },
    { ...task("user-2", "repo-2"), id: "hidden", pinned: false, createdAt: "2026-01-04T00:00:00.000Z" }
  ] as Task[];
  const listed = await listTasksAccessibleToUser({
    listTasks: async (options: { ownerUserId?: string | null; ownerUserIds?: string[]; repositoryIds?: string[] }) =>
      tasks.filter((entry) => options.ownerUserId
        ? entry.ownerUserId === options.ownerUserId
        : options.ownerUserIds?.includes(entry.ownerUserId ?? "") && (!options.repositoryIds || options.repositoryIds.includes(entry.repoId)))
