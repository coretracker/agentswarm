...
describe("task-provider-state", () => {
  ...
  it("builds Claude sidecar config paths", () => {
    ...
    assert.equal(paths.hostPath, path.join(env.TASK_WORKSPACE_DOCKER_SOURCE, ".task-state/task-123/.claude"));
    ...
    assert.equal(paths.configHostPath, path.join(env.TASK_WORKSPACE_DOCKER_SOURCE, ".task-state/task-123/.claude.json"));
  });
