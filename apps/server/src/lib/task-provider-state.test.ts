...
describe("task-provider-state", () => {
  it("builds task-scoped provider state paths", () => {
    ...
    assert.equal(paths.hostPath, path.join(env.TASK_WORKSPACE_HOST_ROOT, ".task-state/task-123/.codex"));
  ...
  it("builds Claude sidecar config paths", () => {
    ...
    assert.equal(paths.hostPath, path.join(env.TASK_WORKSPACE_HOST_ROOT, ".task-state/task-123/.claude"));
    ...
    assert.equal(paths.configHostPath, path.join(env.TASK_WORKSPACE_HOST_ROOT, ".task-state/task-123/.claude.json"));
  });
  ...
  it("builds the task state root path", () => {
    ...
    assert.equal(paths.hostPath, path.join(env.TASK_WORKSPACE_HOST_ROOT, ".task-state/task-abc"));
  });
});
