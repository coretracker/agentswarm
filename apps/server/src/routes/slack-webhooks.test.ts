...
const makeSlackCommandBody = (text: string, overrides: Record<string, string> = {}): string =>
  new URLSearchParams({
    command: "/verft",
    team_id: "T123",
    channel_id: "C12345",
...
test("Slack slash command returns task status with next-step guidance", async () => {
  const app = Fastify();
  addJsonParser(app);
  ...
  registerSlackWebhookRoutes(app, {
    ...
    taskStore: {
      ...
      listPendingActionMessages: async () => [
        {
          id: "message-1",
          content: "also update docs",
          queueSource: "slack_thread",
          externalId: "slack:message:T123:C12345:222.333"
        }
      ]
    } as never,
    scheduler: {
  ...
  const payload = makeSlackCommandBody("status task-1");
  const response = await app.inject({
    method: "POST",
    url: "/slack/commands/repo-1",
    headers: {
  ...
  const body = response.json() as { response_type: string; text: string };
  assert.equal(body.response_type, "ephemeral");
  assert.match(body.text, /Task/);
  assert.match(body.text, /Queued feedback: 1/);
  assert.match(body.text, /What to do: wait for the current run to finish/);
});

test("Slack slash command cancels through the scheduler", async () => {
  const app = Fastify();
  addJsonParser(app);
  ...
  const payload = makeSlackCommandBody("cancel task-1");
  const response = await app.inject({
    method: "POST",
    url: "/slack/commands/repo-1",
    headers: {
  ...
  assert.equal(response.statusCode, 200);
  assert.deepEqual(cancelledTasks, ["task-1"]);
  assert.match((response.json() as { text: string }).text, /Cancellation requested/);
});
